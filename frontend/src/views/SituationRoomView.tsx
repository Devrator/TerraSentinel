    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-white tracking-tight flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-md text-[10px] font-mono bg-rose-500/20 text-rose-300 border border-rose-500/30 uppercase">
                ...
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl border transition-all cursor-pointer ${
                    zone.node_id === selectedNodeId
                      ...
                      : 'bg-[#07110F] border-[#1B2D27] text-slate-300 hover:border-slate-600'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs">{zone.node_id}</span>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-rose-500/30 text-rose-300 font-bold">
                      {zone.severity}
                    </span>
                  </div>
                  <div className="text-[11px] text-slate-400 mt-1 flex items-center gap-2">
                    ...
                    <span>•</span>
                    <span className="font-mono text-rose-400">{zone.score}%</span>
                  </div>
                </button>
              ))
            ) : (
            ...
            {selectedNode ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-[#1B2D27]">
                  <div>
                    <div className="font-bold text-sm text-white">{selectedNode.node_id}</div>
                    <div className="text-[11px] text-slate-400">{selectedNode.name}</div>
                  </div>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${
                    selectedNode.status === 'ONLINE' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-300 border border-rose-500/30'
                  }`}>
                    {selectedNode.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="p-2 rounded-lg bg-[#07110F] border border-[#1B2D27]">
                ...
                <div className="p-3 rounded-xl bg-[#07110F] border border-[#1B2D27] space-y-1.5">
                  <div className="text-[11px] font-bold text-slate-300">Composite Risk Rating</div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-400">AI Hazard Score:</span>
              ...
              <div className="text-xs text-slate-500">Select a node from map</div>
            )}
          </div>
        </div>
      ...
      <div className="p-4 rounded-2xl bg-[#101D19] border border-[#1B2D27]">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] font-mono text-slate-500">REAL-TIME TELEMETRY FEED</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {data?.live_timeline && data.live_timeline.length > 0 ? (
            data.live_timeline.map((event, idx) => (
              <div
                key={idx}
                className="p-3 rounded-xl bg-[#07110F] border border-[#1B2D27] flex flex-col justify-between space-y-1"
              >
                <div className="flex items-center justify-between text-[10px] font-mono">
                  <span className="text-slate-500">{event.timestamp}</span>
                  <span className={`px-1.5 py-0.2 rounded font-bold ${
                    event.severity === 'CRITICAL' ? 'bg-rose-500/20 text-rose-300' : 'bg-amber-500/20 text-amber-300'
                  }`}>
                    {event.node_id}
                  </span>
                </div>
